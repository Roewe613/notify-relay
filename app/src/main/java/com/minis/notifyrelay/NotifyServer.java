...
public class NotifyServer {
    ...
    private static final String LOTTERY_PAGE = "<!doctype html><meta name=viewport content='width=device-width,initial-scale=1'><meta charset=utf-8><title>开奖代理转发</title>"
        ...
        + "<h2>🎲 开奖代理转发</h2><div id=state class=card>加载中…</div>"
        + "<div class=card><h3>开奖网址</h3><label>天津网址<input id=tj></label><label>新疆网址<input id=xj></label></div>"
        + "<div class=card><h3>同步设置</h3><div class=row><label>刷新间隔（分钟）<input id=min type=number min=5 max=60></label><label>获取期数<input id=count type=number min=1 max=20></label></div><button onclick=save()>保存设置</button><button onclick=sync()>立即刷新</button><button onclick=load()>重新载入</button></div>"
